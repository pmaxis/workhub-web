import { computed, onMounted, ref, watch } from 'vue';
import { brainNotesApi, type BrainNote } from '@/features/brain-notes/api/brainNotes.api';
import { useToast } from '@/shared/ui/Toast';

const DEFAULT_LIMIT = 20;
const UNDO_MS = 5000;

export function useBrainNotesList() {
  const { successWithUndo, error: notifyError } = useToast();
  const notes = ref<BrainNote[]>([]);
  const loading = ref(false);
  const error = ref('');
  const deleteTarget = ref<BrainNote | null>(null);
  const search = ref('');

  const page = ref(1);
  const total = ref(0);
  const totalPages = computed(() => Math.max(1, Math.ceil(total.value / DEFAULT_LIMIT)));

  async function load() {
    loading.value = true;
    error.value = '';
    try {
      const result = await brainNotesApi.list({
        page: page.value,
        limit: DEFAULT_LIMIT,
        q: search.value.trim() || undefined,
      });
      notes.value = result.data;
      total.value = result.total;
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : 'Could not load notes';
      error.value = msg;
      notifyError(msg);
      notes.value = [];
      total.value = 0;
    } finally {
      loading.value = false;
    }
  }

  function setPage(p: number) {
    page.value = p;
    void load();
  }

  function promptDelete(n: BrainNote) {
    deleteTarget.value = n;
  }

  function confirmDelete() {
    const n = deleteTarget.value;
    if (!n) return;
    const index = notes.value.findIndex((x) => x.id === n.id);
    deleteTarget.value = null;
    if (index === -1) return;
    const removed = notes.value[index];
    notes.value.splice(index, 1);
    total.value = Math.max(0, total.value - 1);

    const timeoutId = setTimeout(() => {
      void (async () => {
        try {
          await brainNotesApi.remove(removed.id);
          if (notes.value.length === 0 && page.value > 1) {
            page.value -= 1;
          }
          await load();
        } catch (e: unknown) {
          notifyError(e instanceof Error ? e.message : 'Could not delete');
          notes.value.splice(index, 0, removed);
          total.value += 1;
        }
      })();
    }, UNDO_MS);

    successWithUndo(`Note “${removed.title}” deleted`, () => {
      clearTimeout(timeoutId);
      notes.value.splice(index, 0, removed);
      total.value += 1;
    });
  }

  let searchDebounce: ReturnType<typeof setTimeout> | null = null;
  watch(
    () => search.value,
    () => {
      if (searchDebounce) clearTimeout(searchDebounce);
      searchDebounce = setTimeout(() => {
        page.value = 1;
        void load();
      }, 300);
    },
  );

  onMounted(() => {
    void load();
  });

  return {
    notes,
    loading,
    error,
    search,
    page,
    total,
    totalPages,
    setPage,
    deleteTarget,
    promptDelete,
    confirmDelete,
  };
}
